...
class ajax extends Controller
{
    ...
    public function login_form_check(Request $req)
    {
    	...
    	// echo "<script>console.log(".$val.");</script>";
	  	...
	  	if(count($val)>0)
	  	{
	  		...
	  		$blog = DB::select("select id,time,head,blog from blogs where username='$req->username'");
    ...
    public function insert_blog(Request $req)
    {
    	...
    	{
    		$val1 = DB::select("select max(id) as max from blogs");
    		$val = DB::table('blogs')->insert(
    				['id'=>$val1[0]->max+1,'username'=>$req->user,'blog'=>$req->text,'time'=>$req->time,'head'=>$req->head]
    			);
    		if($val)
		  	{
		  		$blog = DB::select("select id,time,head,blog from blogs where username='$req->user'");
	  			$blog = json_encode($blog);
    ...
    public function add_update_data_blog(Request $req)
    {
    	
    	$val = DB::select("select head,blog from blogs where id='$req->id' ");
    ...
    public function final_update_blog(Request $req)
    {
    	
    	$val = DB::table('blogs')
            	->where('id', $req->id)
            	->update(['head' => $req->head,'blog'=>$req->blog]);
